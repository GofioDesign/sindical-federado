...
export function parseMarkdown(source,fileName='article.md'){
  ...
  for(const key of ['title','slug','date','summary','status'])if(!meta[key])throw new Error(`${fileName}: missing ${key}`);
...
export async function loadArticles(root){const dir=join(root,'content/articles');const files=(await readdir(dir)).filter(name=>name.endsWith('.md')&&name!=='TEMPLATE.md');const articles=[];for(const name of files)articles.push(parseMarkdown(await readFile(join(dir,name),'utf8'),name));return articles;}
